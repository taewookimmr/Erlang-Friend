-module(io_widget).
-export([get_state/1,
        start/1, test/0, set_handler/2, set_prompt/2, set_state/2, set_title/2,
        insert_str/2, update_state/3]).
...
widget(Pid)->
    Size = [{widget, 500}, {height, 200}],
    Win = gs:window(gs:start(), 
                [{map, true}, {configure, true}, {titile,"window"}|Size]),
...
loop(Win, Pid, Prompt, State, Parse) -> 
    receive
        ...
        {insert, Str}->
            ...
            loop(Win, Pid, Prompt, State, Parse);
        {updateState, N, X}->
            io:format("setelement N=~p X=~p Satte=~p~n", [N, X, State]),
            State1 = setelement(N,State,X),
            loop(Win, Pid, Prompt, State1, Parse);
        {gs, _, destroy, _, _} ->
            io:format("Destoryed~n", []),
        ...
        {gs, _, configure,[], [W,H,_,_]} ->
            gs:configure(packer, [{width, W}, {height, H}]),
            loop(Win, Pid, Prompt, State, Parse);
...
test1() ->
    ...
    io_widget:set_titile(W, "Test window"),
    loop(w).
